...
### ADR-014 — CRM unificado + campañas de email + membresías + inscripción a eventos (DECIDIDA con salvedades ✅ · 2026-07-28)
- **Contexto:** el negocio quiere (a) que la gente **se anote al evento por el sitio** y que esos datos queden guardados, (b) **enviar campañas de email** desde el panel, (c) **gestionar membresías**, y (d) una base de datos modelada para **reutilizar esos datos a futuro**. Estas features ya estaban en la visión (membresía como unidad de negocio; CRM/inscripciones/métricas en el panel — ADR-011), pero faltaba modelarlas para no construir en silos.
- **Decisión:**
  - **Modelo unificado de Contactos (`Contacts`) en el centro.** Toda persona que toca la marca —compra, se anota al evento, pide presupuesto, es miembro, se suscribe al newsletter— es **UN `Contact`** con `tags`/segmentos y `consent` de email. Las demás entidades (inscripción, orden, membresía, lead) **se relacionan a un `Contact`**, no guardan el email suelto. Esto habilita campañas cruzadas (“a todos los que se anotaron al evento y no compraron”) y **es lo que hace la base reutilizable a futuro**. Evitar duplicar datos de persona en cada colección.
  - **Consentimiento (opt-in) explícito por contacto** — requisito legal para email marketing (Ley 25.326 / AAIP; ver skill `legal-review`). Sin opt-in registrado (con fecha y origen), un contacto **no entra** a campañas.
  - **Campañas de email: mínimo propio en el panel.** Editor simple + envío por la **API de Resend** (mismo proveedor que el transaccional) a **segmentos** del CRM. Nada de ESP externo (Mailchimp/Brevo) por ahora. Colección `EmailCampaigns` con estado y métricas básicas.
  - **Inscripción al evento (18/09): formulario de captación de datos**, no checkout con pago. Crea `EventRegistration` + `Contact` (con opt-in). El **pago del evento se mantiene como hoy** (transferencia / link de MP, **fuera** del sitio). El portal de inscripción **con pago online** queda para una edición futura / su fase normal. *(Recomendación por el deadline de ~7 semanas y por ser Gonzalo principiante: la captura de datos es de bajo riesgo y entrega el objetivo real —datos para campañas—; el checkout con pago es la parte más difícil y no debe apurarse. Franco puede pedir el portal completo si acepta el mayor riesgo/tiempo.)*
  - **Membresías: se modela flexible, cobro PENDIENTE.** Colección `Memberships`/`Plans` colgada de `Contacts`. El **método de cobro queda a definir con Flor** (recurrente automático con MP suscripciones/preapproval vs. manual por link), porque falta definir **qué incluye y cuánto sale** la membresía (checklist §C, carpeta 04_MEMBRESIA).
- **Consecuencia:** amplía el **modelo de datos de la Fase 0 (Tarea 2)** — Gonzalo modela `Contacts`, `EventRegistrations`, `EmailCampaigns`, `Memberships`, y hace que `Orders`/`Quotes` cuelguen de `Contacts` (ver `colaboracion/gonzalo/02_ARQUITECTURA_BACKEND.md` §6). Suma una tarea de **formulario de inscripción al evento** (data-capture) candidata a **priorizarse antes** del checkout de productos, por el deadline del 18/09. Define nuevos módulos del panel: **Comunidad/CRM**, **Campañas**, **Inscripciones**.
- **Estado:** ✅ Decidida en su arquitectura (contactos unificados + email propio por Resend + captura de datos del evento). 🟠 Pendiente: método de cobro de la membresía (con Flor); confirmación de Franco si quiere el portal de evento **con pago** para el 18/09 (hoy: solo captura de datos).
